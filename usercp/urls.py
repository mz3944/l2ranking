...
urlpatterns = patterns('',
    ...
    # Password reset
    url(r'^password/reset/$', 'django.contrib.auth.views.password_reset', {'post_reset_redirect' : 'done/'}, name='password_reset'),
    (r'^password/reset/done/$', 'django.contrib.auth.views.password_reset_done'),
    # This should redirect to /usercp/password/done
    url(r'^password/reset/(?P<uidb36>[0-9A-Za-z]+)-(?P<token>.+)/$', 'django.contrib.auth.views.password_reset_confirm', {'post_reset_redirect' : 'done/'}, name='password_confirm'),
